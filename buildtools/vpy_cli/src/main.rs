...
enum Commands {
    ...
    /// Compile a single `.vmus`/`.vsfx`/`.vec` asset into its byte stream,
    /// emitting a C header (`--format c`) or a raw `.bin` (`--format bin`).
    /// Reuses the exact PSG (`.vmus`/`.vsfx`) / geometry (`.vec`) compiler that
    /// drives the ARM/PiTrex asm backend, so the C (vpy.h) runtime plays and
    /// draws byte-for-byte the same output as hardware.
    CompileAsset {
        /// Input asset file (.vmus, .vsfx or .vec)
        input: PathBuf,
    ...
    let (bytes, suffix) = match ext.as_str() {
        ...
        other => {
            return Err(anyhow::anyhow!(
                "unsupported asset extension '.{other}' (expected .vmus, .vsfx or .vec)"
            ));
        }
    };
