...
enum Commands {
    ...
    /// Compile a single `.vmus`/`.vsfx` asset into its PSG byte stream, emitting
    /// a C header (`--format c`) or a raw `.bin` (`--format bin`). Reuses the
    /// exact notes→PSG compiler that drives the ARM/PiTrex asm backend, so the
    /// C (vpy.h) runtime plays byte-for-byte the same audio as hardware.
    CompileAsset {
        /// Input asset file (.vmus or .vsfx)
        input: PathBuf,
...
/// Compile a `.vmus`/`.vsfx` asset into a C header or raw `.bin`.
///
/// The PSG byte stream comes from the SAME notes→PSG compiler used by the
/// ARM/PiTrex asm backend (`vpy_codegen::pitrex::assets`), so the C runtime's
/// sequencer plays byte-for-byte identical audio to real hardware.
fn cmd_compile_asset(input: &Path, format: &str, out: &Path, name: Option<String>) -> Result<()> {
    ...
    let base = name.unwrap_or_else(|| stem.to_string());
    ...
    let (bytes, suffix) = match ext.as_str() {
        ...
        other => {
            return Err(anyhow::anyhow!(
                "unsupported asset extension '.{other}' (expected .vmus or .vsfx)"
            ));
        }
    };

    println!("  {} → {} PSG bytes", input.display(), bytes.len());
    ...
    match format {
        ...
        "c" | _ => {
            ...
            h.push_str(" * PSG event stream (little-endian) — do not edit by hand. */\n");
