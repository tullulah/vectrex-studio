...
fn cmd_compile_level(input: &Path, format: &str, out: &Path, sym: &str) -> Result<()> {
    ...
    match format {
        ...
        "c" | _ => {
            ...
            let mut h = String::new();
            h.push_str(&format!("/* Auto-generated by `vpy_cli compile-asset` from {}.\n", input.display()));
            h.push_str(" * VPy level byte image (little-endian) + sprite pointer table —\n");
            h.push_str(" * do not edit by hand. Object sprite fields are INDICES into\n");
            h.push_str(&format!(" * {sprites_var}[]; pass both arrays to vpy_load_level(). */\n"));
            h.push_str(&format!("#ifndef {guard}\n#define {guard}\n\n"));
            ...
            h.push_str(&format!("static const unsigned char {level_var}[] = {{\n"));
            for chunk in bytes.chunks(12) {
                let row: Vec<String> = chunk.iter().map(|b| format!("0x{:02X}", b)).collect();
                h.push_str(&format!("    {},\n", row.join(", ")));
            }
            ...
            // Sprite pointer table (index-ordered). C forbids a zero-length
