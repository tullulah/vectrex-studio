...
/// Debug-cart BIOS traps: bus_write / bus_read as SVC stubs into the cartridge
/// BIOS (canonical syscall numbering: firmware syscalls.rs / docs/RP2350_BIOS.md).
/// The BIOS owns the real bus protocol (E-synced, CS-gated). The IDE emulator
/// traps these SYMBOLS before their bodies execute, so it works unchanged.
/// UVM2 does NOT use this — it has its own CLK-synced GPIO protocol.
pub fn emit_bus_helpers() -> String {
    let mut s = String::new();

    s.push_str("@ bus_write(r0=vectrex addr, r1=data) — BIOS trap: SYS_BUS_WRITE\n");
