...
.equ VAR_ROT_X, 0x001005D4
.equ VAR_ROT_Y, 0x001005D8
.equ VAR_ROT_Z, 0x001005DC
.equ VAR_ROT_SPEED_X, 0x001005E0
.equ VAR_ROT_SPEED_Y, 0x001005E4
.equ VAR_ROT_SPEED_Z, 0x001005E8
.equ VAR_JOY_X, 0x001005EC  @ local assign
.equ VAR_JOY_Y, 0x001005F0  @ local assign
...
@ pitrex_draw_vector_3d: Apply Euler X→Y→Z rotation to 3D asset vertices
.global pitrex_draw_vector_3d
.type pitrex_draw_vector_3d, %function
pitrex_draw_vector_3d:
    push    {r4-r11, lr}
    sub     sp, sp, #32         @ locals: sin/cos values + temp workspace
    mov     r4, r0              @ r4 = asset_ptr
    @ Precompute sin_x, cos_x
    mov     r0, r1              @ r0 = rot_x
    bl      pitrex_get_sin
    str     r0, [sp, #0]
    mov     r0, r1              @ r0 = rot_x
    ...
    bl      pitrex_get_cos
    str     r0, [sp, #20]
    ldrb    r5, [r4]            @ r5 = vertex_count
    cmp     r5, #0              @ if no vertices, skip to draw
    beq     .Ldv3d_nodraw
    ldr     r6, [sp, #8]        @ r6 = sin_y
    ldr     r7, [sp, #12]       @ r7 = cos_y
    mov     r10, r5             @ r10 = count (for space calculation)
    add     r10, r10, r10, lsl #1  @ r10 = count * 3
    sub     sp, sp, r10         @ allocate space on stack
    mov     r11, sp             @ r11 = buffer pointer for rotated vertices
    mov     r8, #0              @ r8 = vertex index counter
.Ldv3d_vert_loop:
    cmp     r8, r5              @ done if processed all vertices
    beq     .Ldv3d_verts_done
    mov     r9, r8              @ r9 = index
    add     r9, r9, r9, lsl #1 @ r9 = index * 3
    add     r9, r9, #1          @ r9 += 1 (skip vertex_count byte)
    add     r9, r9, r4          @ r9 = asset + offset
    ldrsb   r0, [r9]            @ r0 = x
    ldrsb   r1, [r9, #1]        @ r1 = y
    ldrsb   r2, [r9, #2]        @ r2 = z
    mov     r3, r0              @ r3 = x
    mov     r0, r7              @ prepare cos_y for multiply (but smul_lut takes in r0, r1)
    mov     r0, r3              @ r0 = x
    mov     r1, r7              @ r1 = cos_y (but smul_lut expects angle in r1)
    mul     r3, r3, r7          @ r3 = x * cos_y
    asr     r3, r3, #7          @ r3 >>= 7 (scale down)
    mul     r12, r0, r6         @ r12 = x * sin_y
    asr     r12, r12, #7        @ r12 >>= 7
    mul     r10, r2, r7         @ r10 = z * cos_y
    asr     r10, r10, #7        @ r10 >>= 7
    mul     r9, r2, r6          @ r9 = z * sin_y
    asr     r9, r9, #7          @ r9 >>= 7
    sub     r3, r3, r9          @ r3 = x*cos_y - z*sin_y
    add     r2, r12, r10        @ r2 = x*sin_y + z*cos_y
    mov     r9, #127
    cmp     r3, r9
    movgt   r3, r9
    mvn     r9, #127            @ r9 = -128
    ...
    mov     r9, r8              @ r9 = index
    add     r9, r9, r9, lsl #1 @ r9 = index * 3
    add     r9, r9, r11         @ r9 = buffer + offset
    strb    r3, [r9]            @ buffer[i*3] = x'
    strb    r1, [r9, #1]        @ buffer[i*3+1] = y (unchanged)
    strb    r2, [r9, #2]        @ buffer[i*3+2] = z'
    add     r8, r8, #1          @ next vertex
    b       .Ldv3d_vert_loop
.Ldv3d_verts_done:
    sub     sp, sp, #256        @ allocate asset copy buffer
    mov     r10, sp             @ r10 = asset_copy pointer
    strb    r5, [r10]           @ buffer[0] = vertex_count
    mov     r8, r5
    add     r8, r8, r8, lsl #1 @ r8 = count*3 bytes
    mov     r9, #0              @ r9 = byte index
.Lrot_copy:
    cmp     r9, r8
    beq     .Lrot_paths
    ldrb    r12, [r11, r9]      @ load rotated vert byte
    add     r3, r10, r9         @ r3 = buffer + i
    strb    r12, [r3, #1]       @ buffer[1+i] = rotated_vert (skip count byte at [0])
    add     r9, r9, #1
    b       .Lrot_copy
.Lrot_paths:
    mov     r8, r5
    add     r8, r8, r8, lsl #1 @ r8 = count*3
    add     r8, r8, #1          @ r8 = path section start offset
    mov     r9, #0
.Lrot_path_copy:
    cmp     r9, #200            @ limit
    beq     .Lrot_draw
    add     r3, r4, r8
    add     r3, r3, r9
    ldrb    r12, [r3]           @ read from asset
    add     r3, r10, r8
    add     r3, r3, r9
    strb    r12, [r3]           @ write to buffer
    cmp     r12, #0x02          @ end marker?
    beq     .Lrot_draw
    add     r9, r9, #1
    b       .Lrot_path_copy
.Lrot_draw:
    mov     r0, r10             @ r0 = asset_copy
    mov     r1, #0              @ r1 = ox (TODO: get from args)
    mov     r2, #0              @ r2 = oy (TODO: get from args)
    mov     r3, #0              @ r3 = mirror
    mov     r7, #127
    push    {r7}                @ intensity
    bl      pitrex_draw_vector_ex
    add     sp, sp, #4
    add     sp, sp, #256        @ deallocate buffer
.Ldv3d_done:
    add     sp, sp, #32
    pop     {r4-r11, pc}
    .ltorg
...
pitrex_game_loop:
    ...
    ldr     r1, =0x001005D4    @ ROT_X
    ldr     r0, [r1]
    push    {r0}
    ldr     r1, =0x001005E0    @ ROT_SPEED_X
    ldr     r0, [r1]
    mov     r1, r0
    pop     {r0}
    add     r0, r0, r1
    ldr     r1, =0x001005D4    @ ROT_X
    str     r0, [r1]
    ldr     r1, =0x001005D8    @ ROT_Y
    ldr     r0, [r1]
    push    {r0}
    ldr     r1, =0x001005E4    @ ROT_SPEED_Y
    ldr     r0, [r1]
    mov     r1, r0
    pop     {r0}
    add     r0, r0, r1
    ldr     r1, =0x001005D8    @ ROT_Y
    str     r0, [r1]
    ldr     r1, =0x001005DC    @ ROT_Z
    ldr     r0, [r1]
    push    {r0}
    ldr     r1, =0x001005E8    @ ROT_SPEED_Z
    ldr     r0, [r1]
    mov     r1, r0
    pop     {r0}
    add     r0, r0, r1
...
if_end_2:
    bl      pitrex_j1_x
    ldr     r1, =0x001005EC    @ JOY_X
    str     r0, [r1]
    bl      pitrex_j1_y
    ldr     r1, =0x001005F0    @ JOY_Y
    str     r0, [r1]
    bl      pitrex_j1_btn1
    cmp     r0, #0
    beq     if_else_3
...
_LOGO_PATH0:
    .byte   127               @ intensity
    .byte   0x41, 0xE9, 0x00, 0x00  @ y=65, x=-23, hdr
...
@ --- LOGO_3D_DATA (1 path(s)) ---
.global _LOGO_3D_DATA
_LOGO_3D_DATA:
    .word   12               @ vertex_count
    .byte   0xE9, 0x3F, 0x00  @ vert 0: x=-23,y=63,z=0
    .byte   0xE9, 0x32, 0x00  @ vert 1: x=-23,y=50,z=0
    .byte   0xDE, 0x32, 0x00  @ vert 2: x=-34,y=50,z=0
    .byte   0xDE, 0x17, 0x00  @ vert 3: x=-34,y=23,z=0
    .byte   0xE9, 0x17, 0x00  @ vert 4: x=-23,y=23,z=0
    .byte   0xE9, 0x0F, 0x00  @ vert 5: x=-23,y=15,z=0
    .byte   0x03, 0x0F, 0x00  @ vert 6: x=3,y=15,z=0
    .byte   0x03, 0x16, 0x00  @ vert 7: x=3,y=22,z=0
    .byte   0x0C, 0x16, 0x00  @ vert 8: x=12,y=22,z=0
    .byte   0x0C, 0x32, 0x00  @ vert 9: x=12,y=50,z=0
    .byte   0x03, 0x32, 0x00  @ vert 10: x=3,y=50,z=0
    .byte   0x03, 0x3F, 0x00  @ vert 11: x=3,y=63,z=0
    .word   1               @ path_count
