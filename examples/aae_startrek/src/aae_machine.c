/* aae_machine.c — Star Trek (Sega G80, Z80). Cloned from the Tac/Scan port; only
 * gamenum, the driver row, the ROM loader and getport differ. SegaG80.c selects
 * this game's port handlers / security / speech by gamenum=STARTREK. */
#include <stdint.h>
#include <string.h>
#include "globals.h"
#include "cpu_control.h"
#include "startrek_roms.h"
...
int              gamenum = STARTREK;
int              WATCHDOG, total_length, testsw, paused;
colors           vec_colors[1024];
aae_settings     config;

struct AAEDriver driver[STARTREK + 1] =
{
    [STARTREK] =
    { "startrek", "Star Trek", 0,
...
void aae_load_roms(void)
{
    GI[CPU0] = gi0_mem;
    GI[1]    = gi1_mem;
    memcpy(GI[CPU0] + 0x0000, startrek_prog, 49152);
    memcpy(GI[1]    + 0x0000, startrek_xyt, sizeof(startrek_xyt));
}
