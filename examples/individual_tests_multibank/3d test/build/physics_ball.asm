...
LOOP_BODY:
    ...
    LDX #_TRIANGLE_3D_DATA  ; pointer to 3D data table
...
; Total paths: 14, points: 56
; X bounds: min=-19, max=19, width=38
; Center: (0, 0)

_TRIANGLE_WIDTH EQU 38
_TRIANGLE_HALF_WIDTH EQU 19
_TRIANGLE_HEIGHT EQU 50
_TRIANGLE_HALF_HEIGHT EQU 25
_TRIANGLE_CENTER_X EQU 0
_TRIANGLE_CENTER_Y EQU 0

_TRIANGLE_VECTORS:  ; Main entry (header + 14 path(s))
    FDB 14               ; path_count (runtime metadata, 2 bytes)
    FDB _TRIANGLE_PATH0        ; pointer to path 0
    FDB _TRIANGLE_PATH1        ; pointer to path 1
    FDB _TRIANGLE_PATH2        ; pointer to path 2
    FDB _TRIANGLE_PATH3        ; pointer to path 3
    FDB _TRIANGLE_PATH4        ; pointer to path 4
    FDB _TRIANGLE_PATH5        ; pointer to path 5
    FDB _TRIANGLE_PATH6        ; pointer to path 6
    FDB _TRIANGLE_PATH7        ; pointer to path 7
    FDB _TRIANGLE_PATH8        ; pointer to path 8
    FDB _TRIANGLE_PATH9        ; pointer to path 9
    FDB _TRIANGLE_PATH10        ; pointer to path 10
    FDB _TRIANGLE_PATH11        ; pointer to path 11
    FDB _TRIANGLE_PATH12        ; pointer to path 12
    FDB _TRIANGLE_PATH13        ; pointer to path 13

_TRIANGLE_PATH0:    ; Path 0
    FCB 127              ; path0: intensity
    FCB $19,$F2,0,0        ; path0: header (y=25, x=-14, relative to center)
...
_TRIANGLE_PATH1:    ; Path 1
    FCB 127              ; path1: intensity
    FCB $17,$F5,0,0        ; path1: header (y=23, x=-11, relative to center)
...
_TRIANGLE_PATH2:    ; Path 2
    FCB 127              ; path2: intensity
    FCB $09,$FD,0,0        ; path2: header (y=9, x=-3, relative to center)
...
_TRIANGLE_PATH3:    ; Path 3
    FCB 127              ; path3: intensity
    FCB $EE,$0B,0,0        ; path3: header (y=-18, x=11, relative to center)
    FCB $FF,$FB,$00          ; flag=-1, dy=-5, dx=0
    FCB $FF,$00,$FC          ; flag=-1, dy=0, dx=-4
    FCB $FF,$05,$00          ; flag=-1, dy=5, dx=0
    FCB $FF,$00,$04          ; flag=-1, dy=0, dx=4
...
_TRIANGLE_PATH4:    ; Path 4
    FCB 127              ; path4: intensity
    FCB $0F,$09,0,0        ; path4: header (y=15, x=9, relative to center)
    FCB $FF,$00,$FA          ; flag=-1, dy=0, dx=-6
    FCB 2                ; End marker (path complete)

_TRIANGLE_PATH5:    ; Path 5
    FCB 127              ; path5: intensity
    FCB $0F,$09,0,0        ; path5: header (y=15, x=9, relative to center)
    FCB $FF,$EE,$00          ; flag=-1, dy=-18, dx=0
    FCB 2                ; End marker (path complete)

_TRIANGLE_PATH6:    ; Path 6
    FCB 127              ; path6: intensity
    FCB $FC,$F1,0,0        ; path6: header (y=-4, x=-15, relative to center)
    FCB $FF,$FB,$00          ; flag=-1, dy=-5, dx=0
    FCB $FF,$00,$11          ; flag=-1, dy=0, dx=17
    FCB 2                ; End marker (path complete)

_TRIANGLE_PATH7:    ; Path 7
    FCB 127              ; path7: intensity
    FCB $19,$F2,0,0        ; path7: header (y=25, x=-14, relative to center)
    FCB $FF,$00,$11          ; flag=-1, dy=0, dx=17
    FCB 2                ; End marker (path complete)

_TRIANGLE_PATH8:    ; Path 8
    FCB 127              ; path8: intensity
    FCB $FD,$ED,0,0        ; path8: header (y=-3, x=-19, relative to center)
    FCB $FF,$12,$00          ; flag=-1, dy=18, dx=0
    FCB 2                ; End marker (path complete)

_TRIANGLE_PATH9:    ; Path 9
    FCB 127              ; path9: intensity
    FCB $F2,$F7,0,0        ; path9: header (y=-14, x=-9, relative to center)
    FCB $FF,$11,$00          ; flag=-1, dy=17, dx=0
    FCB 2                ; End marker (path complete)

_TRIANGLE_PATH10:    ; Path 10
    FCB 127              ; path10: intensity
    FCB $F2,$F7,0,0        ; path10: header (y=-14, x=-9, relative to center)
    FCB $FF,$00,$06          ; flag=-1, dy=0, dx=6
    FCB 2                ; End marker (path complete)

_TRIANGLE_PATH11:    ; Path 11
    FCB 127              ; path11: intensity
    FCB $E7,$0E,0,0        ; path11: header (y=-25, x=14, relative to center)
    FCB $FF,$00,$F0          ; flag=-1, dy=0, dx=-16
    FCB 2                ; End marker (path complete)

_TRIANGLE_PATH12:    ; Path 12
    FCB 127              ; path12: intensity
    FCB $03,$13,0,0        ; path12: header (y=3, x=19, relative to center)
    FCB $FF,$EF,$00          ; flag=-1, dy=-17, dx=0
    FCB 2                ; End marker (path complete)

_TRIANGLE_PATH13:    ; Path 13
    FCB 127              ; path13: intensity
    FCB $09,$FE,0,0        ; path13: header (y=9, x=-2, relative to center)
    FCB $FF,$00,$11          ; flag=-1, dy=0, dx=17
    FCB $FF,$FB,$00          ; flag=-1, dy=-5, dx=0
    FCB 2                ; End marker (path complete)
;***************************************************************************
...
; 3D vertex-indexed data for DRAW_VECTOR_3D (38 unique verts, 14 paths, 56 total point refs)
_TRIANGLE_3D_DATA:
    FDB 38               ; vertex count (unique)
    FCB $F2,$19,$00          ; vert 0: x=-14,y=25,z=0
    FCB $03,$19,$00          ; vert 1: x=3,y=25,z=0
    FCB $03,$0F,$00          ; vert 2: x=3,y=15,z=0
    FCB $09,$0F,$00          ; vert 3: x=9,y=15,z=0
    FCB $09,$FD,$00          ; vert 4: x=9,y=-3,z=0
    FCB $03,$FD,$00          ; vert 5: x=3,y=-3,z=0
    FCB $03,$F7,$00          ; vert 6: x=3,y=-9,z=0
    FCB $F1,$F7,$00          ; vert 7: x=-15,y=-9,z=0
    FCB $F1,$FD,$00          ; vert 8: x=-15,y=-3,z=0
    FCB $ED,$FD,$00          ; vert 9: x=-19,y=-3,z=0
    FCB $ED,$0F,$00          ; vert 10: x=-19,y=15,z=0
    FCB $F2,$0F,$00          ; vert 11: x=-14,y=15,z=0
    FCB $F5,$17,$00          ; vert 12: x=-11,y=23,z=0
    FCB $F9,$17,$00          ; vert 13: x=-7,y=23,z=0
    FCB $F9,$12,$00          ; vert 14: x=-7,y=18,z=0
    FCB $F5,$12,$00          ; vert 15: x=-11,y=18,z=0
    FCB $FD,$09,$00          ; vert 16: x=-3,y=9,z=0
    FCB $0F,$09,$00          ; vert 17: x=15,y=9,z=0
    FCB $0F,$03,$00          ; vert 18: x=15,y=3,z=0
    FCB $13,$03,$00          ; vert 19: x=19,y=3,z=0
    FCB $13,$F2,$00          ; vert 20: x=19,y=-14,z=0
    FCB $0E,$F2,$00          ; vert 21: x=14,y=-14,z=0
    FCB $0E,$F1,$00          ; vert 22: x=14,y=-15,z=0
    FCB $0E,$E7,$00          ; vert 23: x=14,y=-25,z=0
    FCB $FD,$E7,$00          ; vert 24: x=-3,y=-25,z=0
    FCB $FD,$F2,$00          ; vert 25: x=-3,y=-14,z=0
    FCB $F7,$F2,$00          ; vert 26: x=-9,y=-14,z=0
    FCB $F7,$03,$00          ; vert 27: x=-9,y=3,z=0
    FCB $FD,$03,$00          ; vert 28: x=-3,y=3,z=0
    FCB $0B,$EE,$00          ; vert 29: x=11,y=-18,z=0
    FCB $0B,$E9,$00          ; vert 30: x=11,y=-23,z=0
    FCB $07,$E9,$00          ; vert 31: x=7,y=-23,z=0
    FCB $07,$EE,$00          ; vert 32: x=7,y=-18,z=0
    FCB $F1,$FC,$00          ; vert 33: x=-15,y=-4,z=0
    FCB $02,$F7,$00          ; vert 34: x=2,y=-9,z=0
    FCB $FE,$E7,$00          ; vert 35: x=-2,y=-25,z=0
    FCB $FE,$09,$00          ; vert 36: x=-2,y=9,z=0
    FCB $0F,$04,$00          ; vert 37: x=15,y=4,z=0
    FDB 14               ; path count
    ...
    FCB 27               ; vertex index
    FCB 28               ; vertex index
    FCB 4               ; path 3: point count
    FCB 1               ; path 3: closed flag
    FCB 29               ; vertex index
    FCB 30               ; vertex index
    FCB 31               ; vertex index
    FCB 32               ; vertex index
    ...
    FCB 23               ; vertex index
    FCB 35               ; vertex index
    FCB 2               ; path 12: point count
    FCB 0               ; path 12: closed flag
    FCB 19               ; vertex index
    FCB 20               ; vertex index
    FCB 3               ; path 13: point count
