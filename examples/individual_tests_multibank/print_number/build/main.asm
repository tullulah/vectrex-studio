...
VECTREX_PRINT_TEXT:
    ...
    ; Reset0Int ensures absolute positioning from screen center.
    ; Without it, Moveto_d_7F moves RELATIVE to current beam position.
    LDA #$D0
    TFR A,DP       ; Set Direct Page to $D0 for BIOS
    JSR Reset0Int  ; Zero beam integrators -> absolute coords from center
    LDU VAR_ARG2   ; string pointer (reload after Reset0Int)
...
.PN_AFTER_CONVERT:
    ...
    ; Reset0Int ensures absolute positioning from center (same as PRINT_TEXT)
    LDA #$D0
    TFR A,DP         ; Set Direct Page to $D0 for BIOS
    JSR Reset0Int    ; Zero beam integrators -> absolute coords from center
    LDA >VAR_ARG1+1  ; Y coordinate (reload after Reset0Int)
    LDB >VAR_ARG0+1  ; X coordinate
