/* aae_machine.c — Zektor (Sega G80, Z80). Cloned from the Tac/Scan port; only
 * gamenum, the driver row, the ROM loader and getport differ. SegaG80.c selects
 * this game's port handlers / security / speech by gamenum=ZEKTOR. */
#include <stdint.h>
#include <string.h>
#include "globals.h"
#include "cpu_control.h"
#include "zektor_roms.h"
...
struct AAEDriver driver[ZEKTOR + 1] =
{
    [ZEKTOR] =
    { "zektor", "Zektor", 0,
...
void aae_load_roms(void)
{
    GI[CPU0] = gi0_mem;
    GI[1]    = gi1_mem;
    memcpy(GI[CPU0] + 0x0000, zektor_prog, 45056);
    memcpy(GI[1]    + 0x0000, zektor_xyt, sizeof(zektor_xyt));
}
