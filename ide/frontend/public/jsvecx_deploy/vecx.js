...
function VecX()
{
    ...
    this.read8 = function( address )
    {
        ...
        if( address < 0x8000 )
        {
            // Multi-bank ROM support (fallback path for addresses not handled above)
            if (this.isMultibank) {
                const bankSize = 0x4000;
                const numBanks = Math.floor(this.multibankRom.length / bankSize);
                if (address < 0x4000) {
                    return this.multibankRom[(this.currentBank * bankSize) + address] & 0xff;
                } else {
                    return this.multibankRom[((numBanks - 1) * bankSize) + (address - 0x4000)] & 0xff;
                }
            }
