/*
 * AY-3-8910 / YM2149 synth as an AudioWorkletProcessor.
 *
 * The synth math is ported verbatim from JSVecX's e8910.js (which itself is
 * based on the MAME AY core). The critical difference: this runs on the AUDIO
 * RENDER THREAD, not the main thread. e8910's ScriptProcessorNode callback runs
 * on the main thread, so a zero-period infinite loop in the chip emulation
 * freezes the whole UI. Here, worst case, audio glitches — the IDE stays alive.
 *
 * Register writes arrive via port.postMessage({ reg, val }). Periods are still
 * clamped to >= 1 on write (as in e8910), plus primed at construction, so in
 * practice the loops always terminate.
 */
class AY8910Processor extends AudioWorkletProcessor {
  constructor() {
    ...
    this.psg = {
      ...
      RNG: 1, VolTable: new Array(32), Regs: new Array(16).fill(0),
      ready: 0,
    };
    this.buildMixerTable();
    // Prime tone/noise/envelope periods to >= 1 (defensive; see header).
    [0, 2, 4, 6, 11].forEach((r) => this.write(r, 0));
    this.psg.ready = 1;
    this.port.onmessage = (e) => {
      const d = e.data;
      if (d && d.type === 'reg') this.write(d.reg & 0x0f, d.val & 0xff);
      else if (d && d.type === 'reset') { this.psg.Regs.fill(0); [0, 2, 4, 6, 11].forEach((r) => this.write(r, 0)); }
    };
  }
  ...
  callback(stream, length) {
    let idx = 0;
    let outn = 0;
    if (!this.psg.ready) { for (let i = 0; i < length; i++) stream[i] = 0; return; }
    length = length << 1;
    if (this.psg.Regs[7] & 0x01) { if (this.psg.CountA <= length) this.psg.CountA += length; this.psg.OutputA = 1; }
    else if (this.psg.Regs[8] == 0) { if (this.psg.CountA <= length) this.psg.CountA += length; }
    if (this.psg.Regs[7] & 0x02) { if (this.psg.CountB <= length) this.psg.CountB += length; this.psg.OutputB = 1; }
    else if (this.psg.Regs[9] == 0) { if (this.psg.CountB <= length) this.psg.CountB += length; }
    if (this.psg.Regs[7] & 0x04) { if (this.psg.CountC <= length) this.psg.CountC += length; this.psg.OutputC = 1; }
    else if (this.psg.Regs[10] == 0) { if (this.psg.CountC <= length) this.psg.CountC += length; }
    if ((this.psg.Regs[7] & 0x38) == 0x38) { if (this.psg.CountN <= length) this.psg.CountN += length; }
    ...
    while (length > 0) {
      ...
      do {
        let nextevent;
        if (this.psg.CountN < left) nextevent = this.psg.CountN; else nextevent = left;
        if (outn & 0x08) {
          if (this.psg.OutputA) vola += this.psg.CountA;
          this.psg.CountA -= nextevent;
          while (this.psg.CountA <= 0) {
            this.psg.CountA += this.psg.PeriodA;
            if (this.psg.CountA > 0) { this.psg.OutputA ^= 1; if (this.psg.OutputA) vola += this.psg.PeriodA; break; }
            this.psg.CountA += this.psg.PeriodA;
          ...
          while (this.psg.CountA <= 0) {
            this.psg.CountA += this.psg.PeriodA;
            if (this.psg.CountA > 0) { this.psg.OutputA ^= 1; break; }
            this.psg.CountA += this.psg.PeriodA;
          }
        }
        if (outn & 0x10) {
          ...
          while (this.psg.CountB <= 0) {
            ...
            if (this.psg.CountB > 0) { this.psg.OutputB ^= 1; if (this.psg.OutputB) volb += this.psg.PeriodB; break; }
            this.psg.CountB += this.psg.PeriodB;
          ...
          while (this.psg.CountB <= 0) {
            this.psg.CountB += this.psg.PeriodB;
            if (this.psg.CountB > 0) { this.psg.OutputB ^= 1; break; }
            this.psg.CountB += this.psg.PeriodB;
          }
        }
        if (outn & 0x20) {
          ...
          while (this.psg.CountC <= 0) {
            ...
            if (this.psg.CountC > 0) { this.psg.OutputC ^= 1; if (this.psg.OutputC) volc += this.psg.PeriodC; break; }
            this.psg.CountC += this.psg.PeriodC;
          ...
          while (this.psg.CountC <= 0) {
            this.psg.CountC += this.psg.PeriodC;
            if (this.psg.CountC > 0) { this.psg.OutputC ^= 1; break; }
            this.psg.CountC += this.psg.PeriodC;
          }
        }
        this.psg.CountN -= nextevent;
        if (this.psg.CountN <= 0) {
          if ((this.psg.RNG + 1) & 2) { this.psg.OutputN = (~this.psg.OutputN & 0xff); outn = (this.psg.OutputN | this.psg.Regs[7]); }
          if (this.psg.RNG & 1) this.psg.RNG ^= 0x24000;
          this.psg.RNG >>= 1;
      ...
      if (this.psg.Holding == 0) {
        ...
        if (this.psg.CountE <= 0) {
          do { this.psg.CountEnv--; this.psg.CountE += this.psg.PeriodE; } while (this.psg.CountE <= 0);
          if (this.psg.CountEnv < 0) {
            if (this.psg.Hold) {
              if (this.psg.Alternate) this.psg.Attack ^= 0x1f;
              this.psg.Holding = 1;
              this.psg.CountEnv = 0;
            } else {
              if (this.psg.Alternate && (this.psg.CountEnv & 0x20)) this.psg.Attack ^= 0x1f;
              this.psg.CountEnv &= 0x1f;
            }
          }
          this.psg.VolE = this.psg.VolTable[this.psg.CountEnv ^ this.psg.Attack];
      ...
      if (--length & 1) { stream[idx++] = vol / 0x0fff; }
    }
  }

  process(_inputs, outputs) {
    const out = outputs[0] && outputs[0][0];
    if (out) this.callback(out, out.length);
