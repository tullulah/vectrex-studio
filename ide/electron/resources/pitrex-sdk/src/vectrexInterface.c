...
/***********************************************************************/
/*  Smooth continuous drawing — beam stays ON between all segments.
 *
 *  Unlike chaining v_directDraw32 calls (which switch the beam off and
 *  back on at every vertex), these functions keep the beam lit throughout
 *  the entire stroke.  The result is smooth curves with no bright vertex
 *  spots and no stop-start velocity discontinuity between segments.
 *
 *  Coordinate system: same units as v_directDraw32 input (i.e. the VPy
 *  ARM codegen pre-multiplies VPy coords by 127 before passing them in,
 *  just as it does for every other draw call).  The Bézier variants take
 *  raw VPy coordinates and scale internally.
 */

/* de Casteljau linear interpolation — exact integer, no overflow for
 * |a|,|b| <= 16 129 (= 127²) and steps <= 64. */
static int32_t dc_lerp (int32_t a, int32_t b, int i, int steps) {
  return ((int32_t)(steps - i) * a + (int32_t)i * b) / steps;
}

/* Single cubic Bézier point via de Casteljau. */
static int32_t dc_cubic (int32_t p0, int32_t p1, int32_t p2, int32_t p3,
                          int i, int steps) {
...
/* Single quadratic Bézier point via de Casteljau. */
...
/* Apply global sizeX/sizeY/offsetX/offsetY/orientation transform in place. */
static void apply_transform (int32_t *x, int32_t *y) {
  int32_t sx = *x * sizeX + offsetX;
  int32_t sy = *y * sizeY + offsetY;
  if (orientation == 1)      { *x =  sy; *y = -sx; }
  else if (orientation == 2) { *x = -sx; *y = -sy; }
  else if (orientation == 3) { *x = -sy; *y =  sx; }
  else                       { *x =  sx; *y =  sy; }
}

/* Position beam to (x,y) without lighting it.
 * x,y must already be in transformed hardware coordinates. */
static void position_beam_unlit (int32_t x, int32_t y) {
  int32_t xMoveDif = x - currentCursorX;
  int32_t yMoveDif = y - currentCursorY;

  if ((ABS (xMoveDif) > POSITION_MARGIN) || (ABS (yMoveDif) > POSITION_MARGIN)
      || (optimizationON == 0)) {
    int resetPos = ((int)consecutiveDraws > (int)MAX_CONSECUTIVE_DRAWS)
                 + (ABS ((int)currentScale - (int)GET_OPTIMAL_SCALE (xMoveDif, yMoveDif)) > 20)
                 + ((ABS (xMoveDif) > 7000) || (ABS (yMoveDif) > 7000))
                 + (optimizationON == 0);

    if (resetPos) {
      ZERO_AND_WAIT ();
      UNZERO ();
      v_directDeltaMove32start_nosizing (x, y);
      consecutiveDraws = 0;
    } else {
      UNZERO ();
      v_directDeltaMove32start_nosizing (xMoveDif, yMoveDif);
...
void v_directDrawPolyline (const int32_t *xs, const int32_t *ys, int n, uint8_t brightness) {
  ...
  int32_t px = xs[0], py = ys[0];
  apply_transform (&px, &py);
  position_beam_unlit (px, py);
  currentCursorX = px;
  currentCursorY = py;
  ...
  for (int i = 1; i < n; i++) {
    int32_t nx = xs[i], ny = ys[i];
    apply_transform (&nx, &ny);

    int32_t dx = nx - currentCursorX;
    int32_t dy = ny - currentCursorY;

    SET_OPTIMAL_SCALE (dx, dy);
    SET_YSH16 (dy);
    SET_XSH16 (dx);
    START_T1_TIMER ();
    currentCursorX = nx;
...
/* VPy coordinate scale factor — matches the ×127 pre-multiply the VPy
 * ARM codegen applies before calling v_directDraw32 et al. */
#define BEZIER_VPY_SCALE 127

void v_drawBezierCubic (int32_t x0, int32_t y0,
  ...
  for (int i = 1; i <= steps; i++) {
    ...
    int32_t dx = px - currentCursorX;
    int32_t dy = py - currentCursorY;
  ...
  SWITCH_BEAM_OFF ();
  consecutiveDraws += steps + 1;
}

void v_drawBezierQuad (int32_t x0, int32_t y0,
