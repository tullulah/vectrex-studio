...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  runCompile: (args: { path: string; saveIfDirty?: { content: string; expectedMTime?: number }; autoStart?: boolean; compilerBackend?: 'buildtools' | 'core'; target?: 'm6809' | 'rp2350' | 'pitrex' | 'uvm2' }) => ipcRenderer.invoke('run:compile', args) as Promise<{ ok?: boolean; error?: string; binPath?: string; size?: number; stdout?: string; stderr?: string; conflict?: boolean; currentMTime?: number }>,
...
contextBridge.exposeInMainWorld('eprom', {
  ...
  write: (args: { binPath: string; chip: string; programmer: string; skipIdCheck?: boolean; skipVerify?: boolean; eraseFirst?: boolean; unprotect?: boolean; vpp?: string; vdd?: string; vcc?: string; pulse?: string }) =>
  ...
  erase: (args: { chip: string; programmer: string; skipIdCheck?: boolean; unprotect?: boolean }) =>
    ipcRenderer.invoke('eprom:erase', args) as Promise<{ ok: boolean; stdout?: string; stderr?: string; error?: string }>,
  ...
  onProgress: (cb: (line: string) => void) => {
    const handler = (_e: IpcRendererEvent, data: string) => cb(data);
    ipcRenderer.on('eprom://progress', handler);
    return () => ipcRenderer.removeListener('eprom://progress', handler);
  },
});
